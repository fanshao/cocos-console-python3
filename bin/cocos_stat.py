...
def get_user_agent():
    ...
    if cocos.os_is_win32():
        ...
        if cocos.os_is_32bit_windows():
            ...
            arch_str = "WOW64"
        ret_str = (
            "Mozilla/5.0 (Windows NT %s; %s) Chrome/33.0.1750.154 Safari/537.36"
            % (ver_str, arch_str)
        )
    elif cocos.os_is_mac():
        ver_str = (platform.mac_ver()[0]).replace(".", "_")
        ret_str = (
            "Mozilla/5.0 (Macintosh; Intel Mac OS X %s) Chrome/35.0.1916.153 Safari/537.36"
            % ver_str
        )
    elif cocos.os_is_linux():
        arch_str = platform.machine()
        ret_str = (
            "Mozilla/5.0 (X11; Linux %s) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/31.0.1636.0 Safari/537.36"
            % arch_str
        )
...
def gen_bi_event(event, event_value):
    ...
    event_name = category
    params = {"cached_event": is_cache_event}
    if category == "cocos":
        if action == "start":
...
def get_params_str(
    event, event_value, is_ga=True, multi_events=False, engine_version=""
):
    if is_ga:
...
def do_http_request(
    ...
    try:
        params_str = get_params_str(
            event, event_value, is_ga, multi_events, engine_version
        )
        if is_ga:
            host_url = GA_HOST
            host_path = GA_PATH
