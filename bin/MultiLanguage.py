...
class MultiLanguage(object):
    ...
    def get_string(cls, key, fmt_value=None):
        ...
        if fmt_value is None:
            ...
            if isinstance(fmt_value, tuple):
                ...
                for value in fmt_value:
                    if isinstance(value, str):
                        dst_values.append(
                            value.encode(cls.get_instance().get_encoding())
                        )
                    else:
            ...
            elif isinstance(fmt_value, list):
                dst_values = []
                for value in fmt_value:
                    if isinstance(value, str):
                        dst_values.append(value)
                        ...
                        dst_values.append(
                            value.decode(cls.get_instance().get_encoding())
                        )
                ret = fmt % " ".join(dst_values).encode(
                    cls.get_instance().get_encoding()
                )
            else:
                # TODO modify by fsq don't understand why use %
                ret = fmt * fmt_value.encode(cls.get_instance().get_encoding())
                # ret = fmt
    ...
    def __init__(self):
        ...
        if os.path.isfile(cfg_file_path):
            ...
            if MultiLanguage.DEFAULT_LANGUAGE in self.cfg_info:
                self.default_lang_strings = self.cfg_info[
                    MultiLanguage.DEFAULT_LANGUAGE
                ]
            else:
    ...
    def has_key(self, key, strings_info):
        ret = False
        # print(f"key is {key}, strings_info is {strings_info}")
    ...
    def set_current_language(self, lang):
        if (self.cfg_info is not None) and (lang in self.cfg_info):
            ...
            cocos.Logging.warning(
                MultiLanguage.get_string("COCOS_WARNING_LANG_NOT_SUPPORT_FMT", lang)
            )
