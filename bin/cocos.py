...
class Cocos2dIniParser:
                        ...
                        print(
                            (
                                MultiLanguage.get_string(
                                    "COCOS_PARSE_PLUGIN_WARNING_FMT", classname
                                )
                            )
                        )
                    if len(category) == 0:
    ...
    def _sanitize_path(self, path):
        ...
        if not os.path.isdir(path):
            Logging.warning(
                MultiLanguage.get_string("COCOS_WARNING_INVALID_DIR_IN_INI_FMT", path)
            )
            return None
        return path
    ...
    def get_cocos2dx_mode(self):
        ...
        if mode not in ("source", "precompiled", "distro"):
            Logging.warning(
                MultiLanguage.get_string("COCOS_WARNING_INVALID_MODE_FMT", mode)
            )
            mode = "source"
...
class CMDRunner(object):
    ...
    def run_cmd(command, verbose, cwd=None):
        if verbose:
            Logging.debug(
                MultiLanguage.get_string(
                    "COCOS_DEBUG_RUNNING_CMD_FMT", "".join(command)
                )
            )
        else:
        ...
        if ret != 0:
            message = MultiLanguage.get_string(
                "COCOS_ERROR_RUNNING_CMD_RET_FMT", str(ret)
            )
            if not verbose:
                message += MultiLanguage.get_string(
                    "COCOS_ERROR_CHECK_LOG_FMT", log_path
                )
    ...
    def output_for(command, verbose):
        if verbose:
            Logging.debug(
                MultiLanguage.get_string("COCOS_DEBUG_RUNNING_CMD_FMT", command)
            )
        else:
            log_path = CCPlugin._log_path()
        ...
        if cls.inited == False:
            ...
            if cls.stat_obj is not None:
                ...
                local_cfg_file = os.path.join(
                    os.path.expanduser("~/.cocos"), "local_cfg.json"
                )
...
class CCPlugin(object):
    ...
    def get_templates_paths(cls):
        ...
        if path is not None:
            ...
            for p in possible_paths:
                ...
                try:
                    ...
                    Logging.info(
                        MultiLanguage.get_string(
                            "COCOS_INFO_CHECK_TEMPLATE_PATH_FAILED_FMT", template_path
                        )
                    )
                    Logging.info("%s" % e)
                    pass
        ...
        if len(paths) == 0:
            raise CCPluginError(
                MultiLanguage.get_string("COCOS_ERROR_TEMPLATE_NOT_FOUND"),
    ...
    def init(self, args):
        ...
        self._platforms = cocos_project.Platforms(
            self._project, args.platform, args.proj_dir
        )
...
def _check_dependencies_exist(dependencies, classes, plugin_name):
    for dep in dependencies:
        if dep not in classes:
            raise CCPluginError(
                MultiLanguage.get_string(
                    "COCOS_ERROR_INVALID_DEPENDENCY_FMT", (plugin_name, dep)
                ),
...
def get_xcode_version():
    ...
    if xcode is None:
        raise CCPluginError(
            MultiLanguage.get_string("COCOS_ERROR_XCODE_NOT_INSTALLED"),
...
def help():
    print(
        (
            MultiLanguage.get_string(
                "COCOS_HELP_BRIEF_FMT", (sys.argv[0], COCOS2D_CONSOLE_VERSION)
            )
        )
    )
    ...
    max_name = max(
        len(classes[key].plugin_name() + classes[key].plugin_category())
    ...
    print(
        (
            MultiLanguage.get_string(
                "COCOS_HELP_AVAILABLE_ARGS_FMT", MultiLanguage.get_available_langs()
            )
        )
    )
        ...
        if command in plugins:
            ...
            if len(sys.argv) > 2:
                ...
                if command in plugins:
                    ...
                    raise CCPluginError(
                        MultiLanguage.get_string(
                            "COCOS_ERROR_CMD_NOT_FOUND_FMT", " ".join(sys.argv[1:])
                        ),
                        CCPluginError.ERROR_CMD_NOT_FOUND,
                    )
            else:
                raise CCPluginError(
                    MultiLanguage.get_string("COCOS_ERROR_CMD_NOT_FOUND_FMT", command),
                    CCPluginError.ERROR_CMD_NOT_FOUND,
                )
