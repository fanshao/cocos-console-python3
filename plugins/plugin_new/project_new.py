...
print(f"in project_new path is {sys.path}")
...
class CCPluginNew(cocos.CCPlugin):
    ...
    def init(self, args):
        ...
        print(f"type of _projdir is {type(self._projdir)}")
        ...
        if args.engine_path is not None:
            ...
            self._cocosroot = self._cocosroot.decode("utf-8")
    ...
    def parse_args(self, argv):
        ...
        parser = ArgumentParser(
            prog="cocos %s" % self.__class__.plugin_name(),
        ...
        parser.add_argument(
            ...
            nargs="?",
            help=MultiLanguage.get_string("NEW_ARG_NAME"),
        )
        parser.add_argument(
            "-p",
            "--package",
            metavar="PACKAGE_NAME",
            help=MultiLanguage.get_string("NEW_ARG_PACKAGE"),
        )
        parser.add_argument(
            "-d",
            "--directory",
        ...
        parser.add_argument(
            "--ios-bundleid",
            dest="ios_bundleid",
            help=MultiLanguage.get_string("NEW_ARG_IOS_BUNDLEID"),
        )
        parser.add_argument(
            "--mac-bundleid",
            dest="mac_bundleid",
            help=MultiLanguage.get_string("NEW_ARG_MAC_BUNDLEID"),
        )
        parser.add_argument(
            "-e",
            "--engine-path",
        ...
        group = parser.add_argument_group(
            MultiLanguage.get_string("NEW_ARG_GROUP_SCRIPT")
        )
        group.add_argument(
            "--no-native",
            action="store_true",
...
class Templates(object):
        ...
        if current is not None:
            if current in self._template_folders:
                ...
                cocos.Logging.warning(
                    MultiLanguage.get_string("NEW_TEMPLATE_NOT_FOUND_FMT", current)
                )
    ...
    def _scan(self):
        ...
        if len(self._template_folders) == 0:
            ...
            engine_tip = MultiLanguage.get_string(
                "NEW_ERROR_ENGINE_TIP_FMT", need_engine
            )
            message = MultiLanguage.get_string(
...
class TPCreator(object):
    ...
    def cp_self(self, project_dir, exclude_files):
        cocos.Logging.info(
            MultiLanguage.get_string("NEW_INFO_STEP_COPY_TEMPLATE_FMT", project_dir)
        )

        if not os.path.exists(self.project_dir):
            os.makedirs(self.project_dir)
        print(f"in cp_self the {type(self.tp_dir)}, to {type(self.project_dir)}")
        copy_cfg = {
            "from": self.tp_dir,
            "to": self.project_dir,
    ...
    def do_other_step(self, step, not_existed_error=True):
        if step not in self.tp_other_step:
            if not_existed_error:
                ...
                raise cocos.CCPluginError(
                    message, cocos.CCPluginError.ERROR_WRONG_CONFIG
                )
            else:
                # handle as warning
                cocos.Logging.warning(
                    MultiLanguage.get_string("NEW_WARNING_STEP_NOT_FOUND_FMT", step)
                )
                return
    ...
    def do_cmds(self, cmds):
        for k, v in cmds.items():
            ...
            try:
                ...
                raise cocos.CCPluginError(
                    MultiLanguage.get_string("NEW_ERROR_CMD_NOT_FOUND_FMT", k),
    ...
    def project_rename(self, v):
        ...
        cocos.Logging.info(
            MultiLanguage.get_string(
                "NEW_INFO_STEP_RENAME_PROJ_FMT", (src_project_name, dst_project_name)
            )
        )
        files = v["files"]
        for f in files:
    ...
    def project_replace_project_name(self, v):
        ...
        cocos.Logging.info(
            MultiLanguage.get_string(
                "NEW_INFO_STEP_REPLACE_PROJ_FMT", (src_project_name, dst_project_name)
            )
        )
        files = v["files"]
        for f in files:
    ...
    def project_replace_package_name(self, v):
        ...
        cocos.Logging.info(
            MultiLanguage.get_string(
                "NEW_INFO_STEP_REPLACE_PKG_FMT", (src_package_name, dst_package_name)
            )
        )
        files = v["files"]
        if not dst_package_name:
            raise cocos.CCPluginError(
                MultiLanguage.get_string("NEW_ERROR_PKG_NAME_NOT_SPECIFIED"),
    ...
    def project_replace_mac_bundleid(self, v):
        ...
        cocos.Logging.info(
            MultiLanguage.get_string(
                "NEW_INFO_STEP_MAC_BUNDLEID_FMT", (src_bundleid, dst_bundleid)
            )
        )
        files = v["files"]
        for f in files:
    ...
    def project_replace_ios_bundleid(self, v):
        ...
        cocos.Logging.info(
            MultiLanguage.get_string(
                "NEW_INFO_STEP_IOS_BUNDLEID_FMT", (src_bundleid, dst_bundleid)
            )
        )
        files = v["files"]
        for f in files:
    ...
    def modify_files(self, v):
        ...
        for modify_info in v:
            ...
            if not os.path.isabs(modify_file):
                modify_file = os.path.abspath(
                    os.path.join(self.project_dir, modify_file)
                )

            if not os.path.isfile(modify_file):
                cocos.Logging.warning(
                    MultiLanguage.get_string("NEW_WARNING_NOT_A_FILE_FMT", modify_file)
                )
                continue
