...
class CCPluginCompile(cocos.CCPlugin):
    ...
    def _add_custom_options(self, parser):
        ...
        group = parser.add_argument_group(
            MultiLanguage.get_string("COMPILE_ARG_GROUP_ANDROID")
        )
        group.add_argument(
            "--ap",
        ...
        group = parser.add_argument_group(
            MultiLanguage.get_string("COMPILE_ARG_GROUP_WIN")
        )
        group.add_argument(
            "--vs",
            dest="vs_version",
        ...
        group = parser.add_argument_group(
            MultiLanguage.get_string("COMPILE_ARG_GROUP_WEB")
        )
        group.add_argument(
            "--source-map",
            dest="source_map",
        ...
        group = parser.add_argument_group(
            MultiLanguage.get_string("COMPILE_ARG_GROUP_IOS_MAC")
        )
        group.add_argument(
            "-t",
            "--target",
        ...
        group = parser.add_argument_group(
            MultiLanguage.get_string("COMPILE_ARG_GROUP_IOS")
        )
        group.add_argument(
            "--sign-identity",
            dest="sign_id",
        ...
        group = parser.add_argument_group(
            MultiLanguage.get_string("COMPILE_ARG_GROUP_LUA_JS")
        )
        group.add_argument(
            "--no-res",
            dest="no_res",
        ...
        group = parser.add_argument_group(
            MultiLanguage.get_string("COMPILE_ARG_GROUP_LUA")
        )
        group.add_argument(
            "--lua-encrypt",
            dest="lua_encrypt",
    ...
    def _check_custom_options(self, args):
        ...
        self._mode = self.check_param(
            args.mode,
            "debug",
            available_modes,
            MultiLanguage.get_string("COMPILE_ERROR_WRONG_MODE_FMT", available_modes),
        )

        # android arguments
        available_ndk_modes = ["release", "debug", "none"]
        self._ndk_mode = self.check_param(
            args.ndk_mode,
            self._mode,
            available_ndk_modes,
            MultiLanguage.get_string(
                "COMPILE_ERROR_WRONG_NDK_MODE_FMT", available_ndk_modes
            ),
        )
    ...
    def check_param(
        self, value, default_value, available_values, error_msg, ignore_case=True
    ):
        if value is None:
    ...
    def _get_output_dir(self):
        ...
        if self._project._is_script_project():
            ...
            if self._mode == "debug":
                ...
                output_dir = os.path.join(
                    project_dir, CCPluginCompile.OUTPUT_DIR_SCRIPT_RELEASE, cur_platform
                )
        else:
            output_dir = os.path.join(
                project_dir, CCPluginCompile.OUTPUT_DIR_NATIVE, self._mode, cur_platform
            )
    ...
    def compile_lua_scripts(self, src_dir, dst_dir, need_compile=None):
        ...
        cocos_py_path = os.path.join(
            os.path.dirname(os.path.abspath(sys.argv[0])), "cocos.py"
        )
        rm_ext = ".lua"
        compile_cmd = '"%s" "%s" luacompile -s "%s" -d "%s"' % (
            python_path,
            cocos_py_path,
            src_dir,
            dst_dir,
    ...
    def compile_js_scripts(self, src_dir, dst_dir):
        ...
        rm_ext = ".js"
        compile_cmd = '"%s" "%s" jscompile -s "%s" -d "%s"' % (
            python_path,
            cocos_py_path,
            src_dir,
            dst_dir,
    ...
    def build_android(self):
        ...
        cocos.Logging.info(
            MultiLanguage.get_string(
                "COMPILE_INFO_ANDROID_PROJPATH_FMT", (ide_name, project_android_dir)
            )
        )
        ...
        builder = AndroidBuilder(
            self._verbose, project_android_dir, self._no_res, self._project
        )
        ...
        self.apk_path = builder.do_build_apk(
            build_mode,
            self._no_apk,
            self._instant_game,
            output_dir,
    ...
    def check_ios_mac_build_depends(self):
        ...
        if cocos.version_compare(version, "<=", 5):
            ...
            raise cocos.CCPluginError(
                message, cocos.CCPluginError.ERROR_TOOLS_NOT_FOUND
            )
    ...
    def get_engine_dir(self):
        engine_dir = self._project.get_proj_config(
            CCPluginCompile.PROJ_CFG_KEY_ENGINE_DIR
        )
    ...
    def build_ios(self):
        ...
        if self._sign_id is not None:
            cocos.Logging.info(
                MultiLanguage.get_string("COMPILE_INFO_IOS_SIGN_FMT", self._sign_id)
            )
        ...
        section = re.search(
            r"Begin PBXProject section.*End PBXProject section", contents, re.S
        )
        ...
        try:
            ...
            if self._xcworkspace:
                p_name, p_project_name = self.checkFileByExtention(
                    ".xcworkspace", self._platforms.project_path()
                )
            ...
            command = " ".join(
                [
                    ...
                    "%s" % 'VALID_ARCHS="i386 x86_64"'
                    if self.use_sdk == "iphonesimulator"
                    else "",
                ]
            )
            ...
            if self._sign_id is not None:
                ...
                ipa_cmd = 'xcrun -sdk %s PackageApplication -v "%s" -o "%s"' % (
                    self.use_sdk,
                    app_path,
                    ipa_path,
                )
            ...
            raise cocos.CCPluginError(
                MultiLanguage.get_string("COMPILE_ERROR_BUILD_FAILED"),
    ...
    def build_mac(self):
        ...
        if self._project._is_script_project():
            ...
            if self._project._is_lua_project() and (
                self._lua_encrypt or self._compile_script
            ):
                # on iOS, only invoke luacompile when lua encrypt is specified
                self.backup_dir(script_src_dir)
        ...
        if required_versions is None or len(required_versions) == 0:
            if specify_vs_ver is None:
                ...
                if specify_vs_ver < min_ver:
                    ...
                    raise cocos.CCPluginError(
                        MultiLanguage.get_string("COMPILE_ERROR_LOW_VS_VER"),
                        cocos.CCPluginError.ERROR_WRONG_ARGS,
                    )
                else:
            ...
            if specify_vs_ver is None:
                ...
                if specify_vs_ver in required_versions:
                    ...
                    raise cocos.CCPluginError(
                        MultiLanguage.get_string(
                            "COMPILE_ERROR_WRONG_VS_VER_FMT", specify_vs_ver
                        ),
                        cocos.CCPluginError.ERROR_WRONG_ARGS,
                    )
    ...
    def build_vs_project(self, sln_file, project_name, build_mode, specify_vs_ver=None):
        ...
        cocos.Logging.info(
            MultiLanguage.get_string("COMPILE_INFO_REQUIRED_VS_FMT", msg_version)
        )

        needUpgrade, commandPath = self.get_available_devenv(
        ...
        if os.path.exists(commandPath):
            ...
            if needUpgrade:
                commandUpgrade = " ".join(
                    ['"%s"' % commandPath, '"%s"' % sln_file, "/Upgrade"]
                )
            ...
            cocos.Logging.info(
                MultiLanguage.get_string("COMPILE_INFO_DEVENV_NOT_FOUND")
            )

            msbuild_path = self.get_available_msbuild(
                required_versions, min_ver, specify_vs_ver
            )
            ...
            if msbuild_path:
                ...
                cocos.Logging.info(
                    MultiLanguage.get_string(
                        "COMPILE_INFO_FIND_MSBUILD_FMT", msbuild_path
                    )
                )
                ...
                raise cocos.CCPluginError(
                    MultiLanguage.get_string("COMPILE_ERROR_MSDBUILD_NOT_FOUND"),
    ...
    def build_win32(self):
        ...
        if cfg_obj.sln_file is not None:
            ...
            if cfg_obj.project_name is None:
                raise cocos.CCPluginError(
                    MultiLanguage.get_string(
                        "COMPILE_ERROR_CFG_NOT_FOUND_FMT",
                        (
            ...
            if not sln_name:
                ...
                raise cocos.CCPluginError(
                    message, cocos.CCPluginError.ERROR_PATH_NOT_FOUND
                )
        ...
        self.build_vs_project(
            projectPath, self.project_name, build_mode, self.vs_version
        )
        ...
        if not os.path.isdir(build_folder_path):
            message = MultiLanguage.get_string(
                "COMPILE_ERROR_BUILD_PATH_NOT_FOUND_FMT", build_folder_path
            )
        ...
        for filename in files:
            if filename == proj_exe_name:
                ...
                cocos.Logging.info(
                    MultiLanguage.get_string("COMPILE_INFO_COPYING_FMT", filename)
                )
        ...
        if not os.path.exists(build_cfg):
            message = MultiLanguage.get_string(
                "COMPILE_ERROR_FILE_NOT_FOUND_FMT", build_cfg
            )
            raise cocos.CCPluginError(message, cocos.CCPluginError.ERROR_PATH_NOT_FOUND)
        f = open(build_cfg)
        data = json.load(f)
        ...
        if CCPluginCompile.CFG_KEY_MUST_COPY_RESOURCES in data:
            if self._no_res:
                ...
                fileList = (
                    data[CCPluginCompile.CFG_KEY_COPY_RESOURCES]
                    + data[CCPluginCompile.CFG_KEY_MUST_COPY_RESOURCES]
                )
        else:
    ...
    def build_web(self):
        ...
        publish_dir = os.path.normpath(
            os.path.join(
                project_dir, output_dir, CCPluginCompile.WEB_PLATFORM_FOLDER_NAME
            )
        )
    ...
    def build_linux(self):
        ...
        with cocos.pushd(build_dir):
            ...
            self._run_cmd(
                "cmake -DCMAKE_BUILD_TYPE=%s -DDEBUG_MODE=%s %s"
        ...
        if self._project._is_script_project() and self._compile_script:
            cocos.Logging.warning(
                MultiLanguage.get_string("COMPILE_WARNING_NOT_SUPPORT_COMPILE_SCRIPT")
            )
    ...
    def get_wp8_product_id(self, manifest_file):
        ...
        try:
            ...
            raise cocos.CCPluginError(
                MultiLanguage.get_string(
                    "COMPILE_ERROR_MANIFEST_PARSE_FAILED_FMT", manifest_file
                ),
    ...
    def build_wp8(self):
        ...
        build_folder_path = os.path.join(
            wp8_projectdir, cfg_obj.build_folder_path, build_mode
        )
        if not os.path.isdir(build_folder_path):
            message = MultiLanguage.get_string(
                "COMPILE_ERROR_BUILD_PATH_NOT_FOUND_FMT", build_folder_path
    ...
    def run(self, argv, dependencies):
        ...
        cocos.Logging.info(
            MultiLanguage.get_string("COMPILE_INFO_BUILD_MODE_FMT", self._mode)
        )
        ...
        self._project.invoke_custom_step_script(
            cocos_project.Project.CUSTOM_STEP_PRE_BUILD,
            target_platform,
            args_build_copy,
        )
        ...
        self._project.invoke_custom_step_script(
            cocos_project.Project.CUSTOM_STEP_POST_BUILD,
            target_platform,
            args_build_copy,
        )
